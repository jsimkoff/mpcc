    ...
    def _solve(self):
        ...
        while len(queue) > 0:
            ...
            if sense*bound <= sense*incumbent_value:   # TOLERANCE
                # (For minimization:)
                # If the node's LB is lower than the current incumbent, it is worth
                # exploring.
                #

                # Check if integer solution - if so, update incumbent value (new UB)
                # and prune nodes with LB > incumbent, since their children cannot
                ...
                if (not value is None) and (sense*value < sense*incumbent_value):
                    ...
                    print("resetting incumbent bound to %f\n" % incumbent_value)
        ...
        print(subproblem.fixed)
        # incumbent_solution.y.pprint()
        # incumbent_solution.sn.pprint()
        # incumbent_solution.sp.pprint()
